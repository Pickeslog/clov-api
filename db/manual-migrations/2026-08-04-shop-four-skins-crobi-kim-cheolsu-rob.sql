-- 스킨 4종을 상점 카탈로그에 한 번에 등록한다.
--   크로비 '마법학교 학생'   COMMON    900
--   크로비 '개발 경력자'     UNCOMMON  1,500
...
-- 코드가 아니라 이 테이블 데이터다.
--
-- 네 개를 한 파일에 묶은 것은 같은 배치라 근거·검증 쿼리가 공유되기 때문이다. 스킨마다
-- 파일을 나누던 기존 관례에서 벗어나지만, 실행하는 쪽은 한 번만 붙여넣으면 된다.
--
-- 이걸로 스킨이 8종 → 12종이 되고, 마스코트 6종 중 5종이 스킨을 갖는 것은 그대로다
-- (버거노인만 없다 — 본체 상태 세트가 1/9라 순서가 다르다). 대신 크로비가 둘, 롭·김철수가
-- 각각 하나씩 스킨을 더 갖게 된다. 크로비는 이제 3종(오션 구조대·마법학교·개발 경력자)으로
-- 가장 많다.
--
-- ★ 크로비 '개발 경력자'(UNCOMMON)와 김철수 '신입 개발자'(UNCOMMON)는 짝이다. 이름은
--   경력 차이를 말하지만 등급은 같게 뒀다 — 값을 가르는 축은 등급과 내용이지 세계관이
--   아니다. 시니어를 RARE로 올리면 2,800이 되는데, 그러면 '신입 1,500 / 경력 2,800'이
--   되어 같은 날 같은 규격으로 만든 두 스킨에 근거 없는 1,300 차이가 생긴다.
--
-- ══════════════════════════════════════════════════════════════════════
-- ★ 가격 — 등급도 같고 내용도 같으면 값도 같게 둔다
-- ══════════════════════════════════════════════════════════════════════
--
-- 네 개 다 상태 9종을 전부 가진 스킨이다. 그러면 값을 가르는 축은 등급 하나만 남는다.
--
...
VALUES
  ...
  ('COSTUME_CROBI_SENIOR_DEV', '개발 경력자 크로비', '연차가 쌓인 크로비의 개발자 복장',
   'COSTUME', 'UNCOMMON', 1500, 0,
   '/shop/skins/crobi/senior-developer-uncommon/default.png', 240)
...
-- 검증 1: 네 행이 나와야 한다. price 가 900 / 1500 / 1500 / 1500, status 가 전부 ACTIVE 여야 한다.
...
FROM shop_items
WHERE code IN ('COSTUME_CROBI_MAGIC_SCHOOL', 'COSTUME_KIM_CHEOLSU_JUNIOR_DEV',
               'COSTUME_ROB_EXPLORER', 'COSTUME_CROBI_SENIOR_DEV')
ORDER BY sort_order;

-- 검증 2: COMMON · UNCOMMON 두 층의 순서 확인.
-- COMMON   500 · 600 · 900 · 900 · 900
-- UNCOMMON 960 · 1,500 x 5
...
-- 이 배치 뒤 기대값은 77,060 (직전 71,660 + 900 + 1,500 x 3).
-- 하루 총 상한 3,000 기준 완주 일수는 (77,060 - 1,000) / 3,000 ≈ 26일이다.
